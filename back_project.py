...
def back_project(sinogram, order=1, skip=1, use_interp1d=False):
    ...
    for angle in range(angles):
        ...
        # x2 = scipy.ndimage.map_coordinates(sinogram[angle], [x0], order=order, mode='constant', cval=0, prefilter=False)
        # reconstruction = reconstruction + x2 * (math.pi / angles)

        if use_interp1d:
            # Method 1: scipy.interpolate.interp1d (hardcoded to 'linear')
            x2 = scipy.interpolate.interp1d(np.arange(0, ns, 1), sinogram[angle], kind='linear', copy=False, assume_sorted=True, bounds_error=False, fill_value=0, axis=0)
            reconstruction = reconstruction + x2(x0) * (math.pi / angles)
        else:
            # Method 2: scipy.ndimage.map_coordinates
            x2 = scipy.ndimage.map_coordinates(sinogram[angle], [x0], order=order, mode='constant', cval=0, prefilter=False)
            reconstruction = reconstruction + x2 * (math.pi / angles)
